...
@Component({
    ...
    <ul  id="contextMenu2" class="dropdown-menu" role="menu" aria-labelledby="menu1" [ngStyle]="activityContextMenuService.locationCss">
        <li role="presentation" *ngFor="let link of activityContextMenuService.links"> <a role="menuitem" tabindex="-1" (click)="activityContextMenuService.changeStatus(link)">{{link}}</a></li>    
    </ul>`,
    host:{
